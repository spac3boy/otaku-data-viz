...
Production reference content lives in `content/reference-pages/`. `npm run reference:generate` renders each validated content record into `/references/`, adds registry-driven discovery cards to its parent project, cross-links sibling answers from the same dataset, and updates the sitemap. `npm run reference:validate` recomputes the current Pokémon reference claims from the canonical dataset. The normal build publishes only registered generated pages and mirrors them into `/docs`; generated reference pages should not be edited directly.
